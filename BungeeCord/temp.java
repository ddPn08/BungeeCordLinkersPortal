...
public class temp {
    ...
    public static void main(String args[]) {
        try (ServerSocketChannel ssc = ServerSocketChannel.open(); Selector selector = Selector.open();) {
            ...
            ssc.socket().bind(new InetSocketAddress(25566));
            ssc.register(selector, SelectionKey.OP_ACCEPT);
            System.out.println("サーバが起動しました " + ssc.socket().getLocalSocketAddress());
    ...
    private static void doAccept(ServerSocketChannel ssc, Selector selector) {
        try {
    ...
    public static void doRead(SocketChannel channel, Selector selector) {
        try {
            ...
            if (readBytes == -1) {
                System.out.println("disconnected " + channel.socket().getRemoteSocketAddress());
                channel.close();
                return;
            }
            if (readBytes == 0) {
            ...
            System.out.println("recieved " + line + " from " + channel.socket().getRemoteSocketAddress());

            System.out.println("register " + channel.socket().getRemoteSocketAddress());
    ...
    public static void doWrite(SocketChannel channel, Selector selector, byte[] message) {
        try {
            ...
            System.out.println("echo " + line + " to " + channel.socket().getRemoteSocketAddress());
